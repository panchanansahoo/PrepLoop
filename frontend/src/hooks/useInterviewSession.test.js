...
describe('useInterviewSession', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    useAuth.mockReturnValue({ user: { id: 'test-user' }, getAuthHeaders: vi.fn(() => ({})) });
    localStorage.clear();
  });
  ...
  it('handles language change and updates code', () => {
    const { result } = renderHook(() => useInterviewSession());
    act(() => { result.current.actions.handleLanguageChange('javascript'); });
  ...
  it('clears saved session from localStorage', () => {
    localStorage.setItem('ai_interview_session', JSON.stringify({ conversation: [], timestamp: Date.now() }));
    const { result } = renderHook(() => useInterviewSession());
    act(() => { result.current.actions.clearSavedSession(); });
  ...
  it('toggles camera state', () => {
    ...
    act(() => { result.current.actions.toggleCamera(); });
  ...
  it('tracks elapsed time during interview phase', async () => {
    ...
    act(() => { result.current.actions.setPhase('interview'); });
