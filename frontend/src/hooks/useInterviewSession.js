...
export function useInterviewSession() {
    ...
    const [consecutiveSilentQuestions, setConsecutiveSilentQuestions] = useState(0);
    ...
    const [silenceCountdown] = useState(0);
    ...
    useEffect(() => {
        ...
        try {
            window.localStorage.setItem(AI_INTERVIEW_SESSION_KEY, JSON.stringify({
                conversation, questionIndex, currentQuestion,
                elapsed, totalQuestions, interviewType, interviewerGender,
                code, language, notes, timestamp: Date.now(),
            }));
        } catch { }
    }, [conversation, phase, questionIndex, currentQuestion, elapsed, totalQuestions, interviewType, interviewerGender, code, language, notes]);
    ...
    const logAPIError = useCallback((endpoint, stage, error, payload = {}) => {
        ...
        console.error(`[AI Interview API Error] ${stage}:`, { ...errorContext, payloadKeys: Object.keys(payload) });
    }, [totalQuestions, phase]);

    const createHttpError = useCallback((status, statusText = '') => {
        const error = new Error(`HTTP ${status}: ${statusText || 'Request failed'}`);
    ...
    const generateAnalysis = useCallback(async () => {
            ...
            const res = await fetch('/api/company-interview/evaluate', {
                ...
                body: JSON.stringify({
                    conversation: conversation.map(m => ({
                        ...
                        questionSource: m.questionSource, questionMeta: m.questionMeta,
            ...
            if (data && !data.fallback && data.overallScore) {
                ...
                setAnalysisResult({
                    ...data, categories: (data.categories || []).map(cat => ({
                        ...cat, icon: cat.name, color: CATEGORY_ICONS[cat.name]?.color || '#818cf8',
                    })),
                    performanceColor: score >= 7 ? '#22c55e' : score >= 5 ? '#f59e0b' : score >= 3 ? '#f97316' : '#ef4444',
                    stats: statsObj, aiGenerated: true,
                });
        ...
        setAnalysisResult({
            ...
            categories: [
                { name: 'Communication', score: commScore, icon: 'Communication', color: '#818cf8' },
                { name: 'Technical Skills', score: techScore, icon: 'Technical Skills', color: '#22d3ee' },
                { name: 'Problem Solving', score: psScore, icon: 'Problem Solving', color: '#a78bfa' },
                { name: 'Code Quality', score: cqScore, icon: 'Code Quality', color: '#34d399' },
            ],
            strengths: [
            ...
            stats: statsObj, aiGenerated: false,
        });
        setAnalysisLoading(false);
    }, [conversation, elapsed, questionIndex, code, lineCount, language, targetCompany, targetRole, interviewType, getAuthHeaders, totalPauseTime]);
    ...
    useEffect(() => {
        if (phase === 'interview' && !isPaused) {
            questionTimerRef.current = setInterval(() => setQuestionElapsed(prev => prev + 1), 1000);
        }
    ...
    const handleInterviewerCanPlay = useCallback((mode) => {
        ...
        if (interviewerTargetModeRef.current === mode) setInterviewerVisibleMode(mode);
    }, []);
    ...
    useEffect(() => {
        ...
        if (interviewerVideoReady[activeMode]) setInterviewerVisibleMode(activeMode);
        inactiveVideo.pause();
        activeVideo.play().catch(() => { });
        return () => { interviewerPlaybackRef.current[activeMode] = activeVideo.currentTime || 0; };
    ...
    const startInterview = async () => {
        ...
        try {
            ...
            const inferredSkills = SKILL_MAP[interviewType] || ['Software Engineering', 'Problem Solving'];
            ...
            const [res] = await Promise.all([
                fetch('/api/company-interview/start', {
                    ...
                    headers: { 'Content-Type': 'application/json', ...headers },
                    body: JSON.stringify({
                        company: resolvedCompany, role: resolvedRole, stage: resolvedStage,
                        difficulty: experienceLevel === 'fresher' ? 'easy' : 'medium',
                        experienceLevel, totalQuestions, interactionFormat: 'text',
                        advancedOptions: advancedOpts, resumeContext: resumeCtx,
                        interviewerName: 'Interviewer', generateAllQuestions: true,
                    }),
                }),
                minDelay,
            ]);
            ...
            if (!res.ok) {
                logAPIError('/api/company-interview/start', 'interview-start-http', res, { company: resolvedCompany, stage: resolvedStage, experienceLevel });
            ...
            setQuestionIndex(1);
            setConversation([{ role: 'interviewer', content: questionText, timestamp: Date.now() }]);
            ...
            if (statusCode !== 401) logAPIError('/api/company-interview/start', 'interview-start-catch', error, { stage: resolvedStage });
            else console.warn('[AI Interview API Warning] start unauthorized; using local fallback question flow.');
            await minDelay;
            ...
            setQuestionIndex(1);
            setConversation([{ role: 'interviewer', content: fallbackQ, timestamp: Date.now() }]);
    ...
    const sendAnswer = async (isAutoSkip = false, answerOverride = null) => {
        ...
        try {
            ...
            if (silenceStageTimerRef.current) { clearTimeout(silenceStageTimerRef.current); silenceStageTimerRef.current = null; }
            setSilenceStage(0);
            ...
            if (isAutoSkip === true) setConsecutiveSilentQuestions(prev => prev + 1);
            else if (answer && answer.length > 10) setConsecutiveSilentQuestions(0);

            if (answer && answer.length > 10) intelligence.analyzeAnswer(answer, currentQuestion).catch(() => { });
            ...
            const fullAnswer = currentCode ? `${answer}\n\n${CODE_DELIMITER_MARKER}\n${currentCode}` : answer;

            setConversation(prev => [...prev, { role: 'candidate', content: answer || '[Code submitted]', timestamp: Date.now() }]);
            setUserInput('');
            setTranscript('');
            setLoading(true);
            ...
            const pickTechnicalFollowUpFallback = (seed = 0) => technicalFollowUpFallbacks[Math.abs(seed) % technicalFollowUpFallbacks.length];
            ...
            try {
                ...
                const res = await fetch('/api/company-interview/follow-up', {
                    ...
                    headers: { 'Content-Type': 'application/json', ...headers },
                    body: JSON.stringify({
                        previousQuestion: currentQuestion, userAnswer: fullAnswer,
                        questionNumber: questionIndexRef.current + 1, totalQuestions,
                        company: resolvedCompany, role: resolvedRole, stage: resolvedStage,
                        difficulty: experienceLevel === 'fresher' ? 'easy' : 'medium',
                        experienceLevel,
                        conversationHistory: conversation.slice(-12).map(m => ({ role: m.role, content: m.content })),
                        code: code.trim() || undefined, codeLanguage: language,
                        advancedOptions: followUpAdvancedOpts, resumeContext: activeResumeContext,
                    }),
                });

                if (!res.ok) {
                    logAPIError('/api/company-interview/follow-up', 'follow-up-http', res, { stage: resolvedStage, questionIndex, experienceLevel });
                    throw createHttpError(res.status, res.statusText);
                }
                ...
                if (isInterviewOver) {
                    ...
                    setConversation(prev => [...prev, { role: 'interviewer', content: closingText, timestamp: Date.now() }]);
                    ...
                    setConversation(prev => [...prev, { role: 'interviewer', content: earlyEndText, timestamp: Date.now() }]);
                    setLoading(false);
                    ...
                    setConversation(prev => [...prev, { role: 'interviewer', content: continueQ, timestamp: Date.now() }]);
                ...
                const statusCode = getHttpStatus(error);
                if (statusCode !== 401) logAPIError('/api/company-interview/follow-up', 'follow-up-catch', error, { stage: resolvedStage, questionIndex });
                else console.warn('[AI Interview API Warning] follow-up unauthorized; using local fallback feedback flow.');
                ...
                if (isInterviewOverFallback) {
                    ...
                    setConversation(prev => [...prev, { role: 'interviewer', content: closingFallback, timestamp: Date.now() }]);
                    setLoading(false);
                    await speakInterviewerText(closingFallback);
                    setTimeout(() => endInterview(), 1500);
                } else {
                    setConversation(prev => [...prev, { role: 'interviewer', content: fallbackQ, timestamp: Date.now() }]);
                    setCurrentQuestion(fallbackQ);
                    setQuestionIndex(prev => prev + 1);
                    setLoading(false);
                    await speakInterviewerText(fallbackQ);
    ...
    }, [phase, speakInterviewerText, stopSilenceHandling, interviewType, hasRecentSpeech]);

    useEffect(() => {
        if (isListening && !transcript.trim() && !aiSpeaking) startSilenceHandling();
    ...
    const endInterview = useCallback(() => {
        ...
        voiceAI.cleanup();
        try { const stats = voiceAI.getAnalytics(); console.info('[AI Interview] Voice analytics:', stats); } catch { }
        clearSavedSession();
        setAnalysisLoading(true);
        setPhase('summary');
    ...
    return {
        ...
        refs: {
            voiceHookRef, sendAnswerRef, isListeningRef, isSendingRef,
            ttsAudioRef, chatEndRef, silenceStageTimerRef, stateRefs,
            questionIndexRef, timerRef, questionTimerRef, pauseStartRef,
            countdownWarnedRef, speakSequenceCancelledRef, streamRef,
            interviewerSpeakingVideoRef, interviewerListeningVideoRef,
            interviewerPlaybackRef, interviewerTargetModeRef, videoRef,
        },
        actions: {
            setPhase, setInterviewType, setRealtimeMode, setInterviewerGender,
            setActiveTab, setLanguage, setCode, setNotes, setElapsed,
            setIsPaused, setTotalPauseTime, setCurrentQuestion, setQuestionIndex,
            setTotalQuestions, setStageLabel, setStagePlan, setLoading,
            setConsecutiveSilentQuestions, setScoreCue, setActiveHint,
            setAnalysisLoading, setCameraOn, setBookmarked, setSpeakerMuted,
            setChatOpen, setConversation, setUserInput, setInterviewerStatus,
            setSilenceStage, setSetupStep, setExperienceLevel, setTargetRole,
            setTargetCompany, setCompanySearch, setCompanyTab, setResumeFile,
            setActiveResumeContext, setResultTab, setAnalysisResult,
            setExpandedMoment, setCaptionsOn, setWorkspacePanelOpen,
            setWorkspaceDropdownOpen, setSavedSession, setMicOn,
            setInterviewerVideoReady, setInterviewerVisibleMode,
            setTranscript, clearSavedSession, restoreSession,
            ...
            startSilenceHandling, logAPIError, createHttpError, getHttpStatus,
