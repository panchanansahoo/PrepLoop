...
const OVERSCAN    = 5;   // extra rows rendered above/below the viewport
...
function ProblemRow({ problem, idx, isLight, solvedSet, onSolveProblem, getExplanationSnippet }) {
    ...
    const baseBg = isSolved ? 'rgba(110,231,183,0.025)' : idx % 2 === 0 ? (isLight ? 'rgba(0,0,0,0.015)' : 'rgba(255,255,255,0.008)') : 'transparent';

    return (
        <div style={{
            display: 'grid', gridTemplateColumns: '50px 1fr 140px 100px 80px 70px',
            gap: 8, padding: '10px 20px', alignItems: 'center', height: ROW_HEIGHT, boxSizing: 'border-box',
            borderBottom: isLight ? '1px solid rgba(0,0,0,0.05)' : '1px solid rgba(255,255,255,0.03)',
            background: baseBg, transition: 'background 0.15s',
        }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <span style={{
                        ...
                        color: isSolved ? (isLight ? 'rgba(0,0,0,0.35)' : 'rgba(255,255,255,0.4)') : (isLight ? '#1e293b' : '#fff'),
                        textDecoration: isSolved ? 'line-through' : 'none',
                        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                    }}>{problem.title}</span>
                    <span style={{
                        fontSize: 10, color: isLight ? 'rgba(30,41,59,0.55)' : 'rgba(255,255,255,0.45)',
                        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
...
export function ProblemExplorerAllQuestionsView({
    ...
    // Reset scroll position when the problem list changes (filter/sort)
    useEffect(() => {
    ...
    const onSort = (key) => {
        setSortBy(key);
        setSortDir((dir) => (sortBy === key ? (dir === 'asc' ? 'desc' : 'asc') : 'asc'));
    };

    const totalHeight = filteredProblems.length * ROW_HEIGHT;
    ...
    const visibleProblems = filteredProblems.slice(startIdx, endIdx + 1);
    const paddingTop    = startIdx * ROW_HEIGHT;
    const paddingBottom = Math.max(0, (filteredProblems.length - endIdx - 1) * ROW_HEIGHT);
        ...
        <div style={{
            ...
            >
                {filteredProblems.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: 48, color: isLight ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.3)' }}>
