import { buildApiUrl } from "./safeApiUrl";

function buildCodeforcesProxyUrl() {
  const configuredApiUrl = import.meta.env.VITE_API_URL;
  if (!configuredApiUrl) return "/api/contest.list";
  return buildApiUrl("/contest.list", {
    rawBaseUrl: configuredApiUrl,
    apiPrefix: "/api",
  });
}

// ── Codeforces API (live data via backend proxy, direct fallback) ──
export async function fetchCodeforcesContests() {
  const endpoints = [
    buildCodeforcesProxyUrl(),
...
export function getNextDayOfWeek(dayOfWeek, hour = 0, minute = 0) {
  ...
  result.setUTCDate(
    result.getUTCDate() + (diff === 0 && result <= now ? 7 : diff),
  );
...
export async function fetchAllContests() {
  try {
    ...
    return generateScheduledContests()
      .filter((c) => c.date > new Date())
      .sort((a, b) => a.date - b.date);
  }
}
