...
import { dsaPatternsAll } from "../data/dsaPatternsData";
...
const getSolutionUnlockKey = (id) =>
  `dsa-solution-unlocked-${String(id ?? "").trim()}`;

// Enhanced error parser with line number extraction
const parseErrorWithLineInfo = (errorText = "") => {
  const text = String(errorText || "").trimEnd();
...
const getPlaygroundFriendlyError = (rawError = "", status = 0) => {
  ...
  if (
    lower.includes("code must be a string") ||
...
const isLegacyPythonStarter = (value = "") => {
  return (
    value.includes("Hello from PrepLoop Playground!") &&
    value.includes("def hello():")
  );
};
    ...
    if (normalized === "python") {
      ...
      return lines
        .map((line) => {
          ...
          return line
            .slice(
              Math.min(commonIndent, line.match(/^[ \t]*/)?.[0].length || 0),
            )
            .replace(/[ \t]+$/g, "");
        })
        .join("\n")
...
export default function CodingPlayground({ sidebarCollapsed }) {
  ...
  const [language, setLanguage] = useState(() => {
    ...
    return supportedLanguageIds.includes(savedLanguage)
      ? savedLanguage
      : "python";
  });
  ...
  const [editorTheme, setEditorTheme] = useState(() =>
    getSavedTheme("playground-editor-theme"),
  );
  const [showThemeMenu, setShowThemeMenu] = useState(false);

  // ─── NEW FEATURE STATE ───
  const [fontSize, setFontSize] = useState(
    () => parseInt(localStorage.getItem("pg-font-size")) || 14,
  );
  const [execHistory, setExecHistory] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("pg-exec-history") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const [voiceErrorsEnabled, setVoiceErrorsEnabled] = useState(
    () => localStorage.getItem("pg-voice-errors-enabled") === "1",
  );
  ...
  useEffect(() => {
    setSolutionUnlocked(
      localStorage.getItem(getSolutionUnlockKey(problemId)) === "true",
    );
  }, [problemId]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    let found = PROBLEMS.find((p) => String(p.id) === String(problemId));

    // Fallback: search DSA patterns data for the problem
    if (!found) {
      for (const pattern of dsaPatternsAll) {
        const match = (pattern.problems || []).find(
        ...
        if (match) {
          found = {
            ...
            leetcodeLink: match.leetcodeLink || "",
            category: pattern.category || pattern.name || "",
            pattern: pattern.name || "",
            description: `Practice problem from the "${pattern.name}" pattern.`,
            examples: [],
            constraints: [],
          };
          break;
    ...
    if (found) {
      setProblem(found);
      // Always reset to starter code when navigating to a new problem
      const starter = STARTER_CODE[language]
        ? STARTER_CODE[language](found.functionName)
        : "";
  ...
  const handleLanguageChange = (lang) => {
    ...
    if (problem) {
      ...
      if (
        ...
        currentCodeTrimmed.length < 50
      ) {
        const starter = STARTER_CODE[lang]
          ? STARTER_CODE[lang](problem.functionName)
          : "";
        if (starter) setCode(starter);
      }
    }
  ...
  const handleRun = useCallback(async () => {
    ...
    setConsoleOutput((prev) => [
      ...prev,
      { type: "info", text: `[${timestamp}] Running ${language}...` },
    ]);
    setLastExecutionMeta(null);

    runPhaseTimeoutsRef.current.push(
      setTimeout(() => setRunPhase("sending"), 80),
    );
    runPhaseTimeoutsRef.current.push(
      setTimeout(() => setRunPhase("compiling"), 350),
    );
    runPhaseTimeoutsRef.current.push(
      setTimeout(() => setRunPhase("executing"), 900),
    );
    ...
    try {
      ...
      if (!res.ok) {
        const friendlyError = getPlaygroundFriendlyError(
          data?.error,
          res.status,
        );
        setConsoleOutput((prev) => [
      ...
      if (data.success && outputText) {
        ...
        const combinedError =
          errorText || outputText || "Execution failed. Please try again.";
        combinedError.split("\n").forEach((line) => {
          const errorInfo = parseErrorWithLineInfo(line);
          let displayText = getPlaygroundFriendlyError(
            errorInfo.text,
            res.status,
          );
        ...
        outputLines.push({
          type: "info",
          text: "(No output — use console.log() or print() to see results)",
        });
      }
      ...
      const histEntry = {
        ...
        codeSnippet: code.slice(0, 100),
        outputPreview:
          outputText.slice(0, 80) || errorText.slice(0, 80) || "No output",
        success: data.success,
        runtimeMs: totalMs || 0,
      };
      setExecHistory((prev) => {
      ...
      if (err?.name === "AbortError") {
        setConsoleOutput((prev) => [
          ...prev,
          { type: "info", text: "Execution cancelled by user." },
        ]);
      } else {
        setConsoleOutput((prev) => [
          ...prev,
  ...
  const handleFormat = useCallback(async () => {
    ...
    setConsoleOutput((prev) => [
      ...prev,
      { type: "info", text: `✓ Code formatted (${language})` },
    ]);
  }, [code, language]);
  ...
  const handleAiAssist = useCallback(
    async (mode, customPrompt = "") => {
      ...
      try {
        const res = await authFetch(`${API_URL}/api/ai/playground-assist`, {
          method: "POST",
          body: JSON.stringify({
            code,
            language,
  ...
  const handleAiRetry = () => {
    if (lastAiRequest) {
      ...
      setTimeout(
        () => handleAiAssist(lastAiRequest.mode, lastAiRequest.customPrompt),
        100,
      );
    }
  };
  ...
  useEffect(() => {
    const serverLintSupportedLanguages = new Set([
      "python",
      "javascript",
      "c",
      "cpp",
    ...
    if (
      !serverLintSupportedLanguages.has(language) ||
      !String(code || "").trim()
    ) {
  ...
  useEffect(() => {
    localStorage.setItem(
      "pg-voice-errors-enabled",
      voiceErrorsEnabled ? "1" : "0",
    );
  }, [voiceErrorsEnabled]);
  ...
  const speakViaCloud = useCallback(async (text, language) => {
    try {
      ...
      const response = await authFetch(`${API_URL}/api/voice/tts`, {
        method: "POST",
        body: JSON.stringify({
  ...
  const buildVoiceAnnouncement = useCallback(
    (items, language) => {
      ...
      const body = prioritized
        .map((error) => {
          ...
          if (isHindi)
            return `लाइन ${error.line}, कॉलम ${error.col}. ${shortType}: ${spokenMessage}`;
  ...
  useEffect(() => {
    ...
    const announceable = normalized.filter(
    ...
    voiceTimerRef.current = setTimeout(() => {
      const announcement = buildVoiceAnnouncement(announceable, "en");
      if (!announcement) return;
      playVoiceAnnouncement(announcement, "en");
    }, 420);
  ...
  const insertTemplate = (template) => {
    const templateCode =
      template.templates[language] || template.templates.python || "";
  ...
  const sidebarTabs = useMemo(
    () => [
      ...(problem
        ? [{ id: "problem", icon: <FileCode2 size={16} />, label: "Problem" }]
        : []),
      {
        id: "errors",
        icon: <AlertTriangle size={16} />,
        label: `Errors${liveErrors.length ? ` (${liveErrors.length})` : ""}`,
      },
  ...
  const isMobileOverlayOpen =
    isMobileView && (showMobileSidebar || showMobileConsole || showLangMenu);
    ...
    <div className="pg-root" ref={rootRef}>
      ...
      <div className="pg-topbar">
        ...
        <div className="pg-topbar-center pg-topbar-tools">
          ...
          <div className="pg-theme-wrap" style={{ position: "relative" }}>
            ...
            >
              <Palette size={14} />
              <span>
                {EDITOR_THEMES.find((t) => t.id === editorTheme)?.label ||
                  "Theme"}
              </span>
              <ChevronDown size={12} />
            </button>
        ...
        <div className="pg-topbar-center pg-topbar-run">
          ...
          >
            ...
            <span>
              {running ? `Running (${runPhaseLabel})...` : "Run Code"}
            </span>
      ...
      <div className="pg-main-split">
        ...
        <div className="pg-editor-area">
          ...
          {/* Console Panel */}
          <div
            className={`pg-console ${showMobileConsole ? "pg-console-mobile-open" : ""}`}
            style={{ height: consoleHeight }}
          >
            <div
              className="pg-console-resize"
            ...
            <div className="pg-console-header">
              <div className="pg-console-title">
                <Terminal size={13} />
                <span>Console</span>
                <span className="pg-console-count">
            ...
            <div className="pg-console-body">
              {filteredConsoleOutput.length === 0 ? (
                ...
                filteredConsoleOutput.map((line, i) => (
                  <div
                    key={i}
                    className={`pg-console-line pg-console-${line.type}`}
                  >
                    {line.type === "info" && (
                      <span className="pg-console-prefix">›</span>
                    )}
                    {line.type === "output" && (
                      <span className="pg-console-prefix">»</span>
                    )}
        ...
        {/* Right Sidebar */}
        <div
          className={`pg-sidebar ${!isSidebarVisible ? "pg-sidebar-collapsed" : ""} ${isSidebarVisible ? "pg-sidebar-mobile-open" : ""}`}
        >
          <div
          ...
          >
            ...
            >
              ...
              <button
                ...
                title={`Fullscreen (F11) - ${isFullscreen ? "Exit" : "Enter"}`}
              >
                {isFullscreen ? (
                  <Minimize2 size={16} />
                ) : (
              ...
              <div className="pg-sidebar-section">
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setVoiceErrorsEnabled(e.target.checked)
                        }
                      />
                    </label>
                  </div>
                  ...
                  {voiceSupported && voiceErrorsEnabled && (
                    ...
                    >
                      Live errors are read aloud while you type.
                    </div>
                  )}
                </div>
                {liveLintPending && (
                    ...
                    liveErrors.map((error, idx) => (
                      ...
                      >
                        <div className="pg-history-meta">
                          <span>
                            Line {error.line}:{error.col || 1}
                          </span>
                          <span className="pg-history-lang">syntax</span>
                        </div>
                        <div className="pg-history-preview">
                          {error.message}
                        </div>
                      </button>
                    ))
              ...
              <div className="pg-sidebar-section pg-ai-section">
                ...
                <div className="pg-ai-messages">
                  {aiMessages.length === 0 && !aiLoading && (
                    <div className="pg-ai-empty">
                      ...
                      <span>
                        Analyze, debug, optimize, and understand your code with
                        AI
                      </span>
                      <div className="pg-ai-suggestions">
                        {[
                          ...
                          <button
                            ...
                            disabled={aiLoading || !code.trim()}
                          >
                            <span className="pg-ai-suggestion-icon">
                              {s.icon}
                            </span>
                            {s.text}
                          </button>
                ...
                <div className="pg-ai-input-area">
                  ...
                  <button
                    ...
                    onClick={() =>
                      aiInput.trim() && handleAiAssist("ask", aiInput.trim())
                    }
                ...
                <div className="pg-sidebar-scroll">
                  {execHistory.length === 0 ? (
                    ...
                    execHistory.map((entry) => (
                      <div
                        key={entry.id}
                        className={`pg-history-entry ${entry.success ? "" : "pg-history-error"}`}
                      >
                        <div className="pg-history-meta">
                          <span>{entry.timestamp}</span>
                          <span className="pg-history-lang">
                            {entry.language}
                          </span>
                          {typeof entry.runtimeMs === "number" ? (
                            <span>{Math.round(entry.runtimeMs)}ms</span>
                          ) : null}
                          {entry.success ? (
                  ...
                  <div className="pg-info-grid">
                    {[
                      ...
                      [
                        "Last Total",
                        lastExecutionMeta
                          ? `${Math.round(lastExecutionMeta.totalMs)}ms`
                          : "N/A",
                      ],
                      [
                        "Last Compile",
                        lastExecutionMeta
                          ? `${Math.round(lastExecutionMeta.compileMs)}ms`
                          : "N/A",
                      ],
                      [
                        "Last Run",
                        lastExecutionMeta
                          ? `${Math.round(lastExecutionMeta.runMs)}ms`
                          : "N/A",
                      ],
                      [
                        "Compile Cache",
                        lastExecutionMeta
                          ? lastExecutionMeta.cacheHit
                            ? "Hit"
                            : "Miss"
                          : "N/A",
                      ],
      ...
      {isMobileView && showLangMenu && (
        <div
          className="pg-mobile-lang-sheet"
          role="dialog"
          aria-modal="true"
