...
queue = queue[1:]           // dequeue`}},{label:"LinkedList",icon:"🔗",code:{python:`class ListNode:
  ...
  constructor(val = 0, left = null, right = null) {
    ...
    Left  *TreeNode
    Right *TreeNode
}`}}];function hr(s){const n=Math.floor(s/60),i=s%60;return`${n.toString().padStart(2,"0")}:${i.toString().padStart(2,"0")}`}function zr(){var ft;const s=Rs(),n=c.useRef(null),i=Y.map(t=>t.id),[r,l]=c.useState(()=>{const t=localStorage.getItem("playground-lang")||"python";return i.includes(t)?t:"python"}),[a,d]=c.useState(""),[u,m]=c.useState(!1),[y,h]=c.useState([]),[v,w]=c.useState(!1),[N,g]=c.useState(!1),[f,j]=c.useState(!1),[ee,V]=c.useState(!1),[Fe,Ft]=c.useState(200),[He,We]=c.useState(0),[xe,Je]=c.useState(!1),[Ge,ye]=c.useState(!1),[te,Ht]=c.useState(()=>As("playground-editor-theme")),[Ue,be]=c.useState(!1),[ve,Wt]=c.useState(()=>parseInt(localStorage.getItem("pg-font-size"))||14),[se,Ve]=c.useState(()=>{try{return JSON.parse(localStorage.getItem("pg-exec-history")||"[]")}catch{return[]}}),[fr,gr]=c.useState(!1),[xr,Jt]=c.useState(!1),[ne,Gt]=c.useState({line:1,col:1}),[Ut,Be]=c.useState(!1),[Ke]=c.useState(!0),[Xe,Vt]=c.useState(!1),[re,Ye]=c.useState(!1),[J,Ze]=c.useState("errors"),[P,Qe]=c.useState([]),[Bt,je]=c.useState(!1),[et,$]=c.useState("idle"),[L,tt]=c.useState(null),[ie,Kt]=c.useState("all"),[O,B]=c.useState([]),[G,st]=c.useState(""),[A,nt]=c.useState(!1),[rt,it]=c.useState(null),[Ne,at]=c.useState(null),ot=c.useRef(null),Ce=c.useRef(null),q=c.useRef(null),lt=c.useRef(null),ct=c.useRef(null),Ee=c.useRef(null),ut=c.useRef(0),I=c.useRef([]),D=c.useRef(null),ae=c.useRef(0);c.useEffect(()=>{const t=localStorage.getItem(`playground-code-${r}`);d(t||zt[r]||"")},[r]),c.useEffect(()=>{if(!a)return;const t=setTimeout(()=>{localStorage.setItem(`playground-code-${r}`,a)},500);return()=>clearTimeout(t)},[a,r]),c.useEffect(()=>{if(!xe)return;const t=setInterval(()=>We(o=>o+1),1e3);return()=>clearInterval(t)},[xe]),c.useEffect(()=>{var t;(t=ct.current)==null||t.scrollIntoView({behavior:"smooth"})},[y]);const Xt=t=>{l(t),localStorage.setItem("playground-lang",t),g(!1)},K=c.useCallback(async()=>{var C;(C=D.current)==null||C.abort(),ae.current+=1;const t=ae.current,o=new AbortController;D.current=o,I.current.forEach(x=>clearTimeout(x)),I.current=[],$("queued"),m(!0);const p=new Date().toLocaleTimeString();h(x=>[...x,{type:"info",text:`[${p}] Running ${r}...`}]),tt(null),I.current.push(setTimeout(()=>$("sending"),80)),I.current.push(setTimeout(()=>$("compiling"),350)),I.current.push(setTimeout(()=>$("executing"),900));try{const x=await fetch(`${Ae}/api/practice/execute`,{method:"POST",headers:Ie(),body:JSON.stringify({code:a,language:r}),signal:o.signal});if(t!==ae.current)return;const b=await x.json().catch(()=>({}));if($("processing"),!x.ok){const S=It(b==null?void 0:b.error,x.status);h(H=>[...H,{type:"error",text:S}]);const M={id:Date.now(),timestamp:p,language:r,codeSnippet:a.slice(0,100),outputPreview:S.slice(0,80),success:!1};Ve(H=>{const gt=[M,...H].slice(0,10);return localStorage.setItem("pg-exec-history",JSON.stringify(gt)),gt});return}const T=(b.output||"").trim(),z=(b.error||"").trim(),R=[],E=Number(b.compileTime||0),ue=Number(b.runTime||0),F=Number(b.executionTime||0),de=!!b.cacheHit;tt({success:!!b.success,compileMs:E,runMs:ue,totalMs:F,cacheHit:de,language:r,time:p}),b.success&&T?T.split(`
