...
const MAX_LINT_CODE_LENGTH = Number.parseInt(process.env.MAX_LINT_CODE_LENGTH || '100000', 10);
...
const awardFirstSolveCoins = async ({ userId, problemId, problemTitle }) => {
  const description = `Problem solved: ${problemId} - ${problemTitle || 'Unknown Problem'}`.slice(
    0,
    160
  );
...
const detectFunctionName = (starterCode = '', userCode = '', language = '') => {
  ...
  if (language === 'python') {
    ...
    const solutionMethodMatch = code.match(/class\s+Solution[\s\S]*?def\s+([A-Za-z_]\w*)\s*\(/);
...
const getJudgeProfile = (problemTitle = '', fnName = '') => {
  ...
  const normalizedFnName = String(fnName || '')
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
...
const normalizeUnorderedFlatArray = (arr) => arr.map((item) => stableStringify(item)).sort();

const normalizeUnorderedNestedOrderedArray = (arr) =>
  arr.map((item) => stableStringify(item)).sort();
...
const extractPythonSyntaxError = (stderr = '') => {
  ...
  const syntaxLine =
    lines.find((lineText) => lineText.trim().startsWith('SyntaxError')) || 'Syntax error';
  return {
...
const extractCompilerSyntaxErrors = (stderr = '', fallbackMessage = 'Syntax error') => {
  ...
  const firstMeaningfulLine = lines
    ...
    .find(
      (lineText) =>
        lineText && !lineText.startsWith('^') && !lineText.includes('compilation terminated')
    );
...
const evaluateWithStdinCases = async (code, language, testCases = [], judgeProfile = 'strict') => {
  ...
  for (const tc of testCases) {
    ...
    const actual = execResult.success
      ? normalizeTextOutput(execResult.output)
      : execResult.error || 'Runtime Error';
...
const resolveProblemRecord = async (problemIdentifier) => {
  ...
  const identifierValue =
    typeof problemIdentifier === 'object' && problemIdentifier !== null
      ? (problemIdentifier.id ??
        problemIdentifier.problemId ??
        problemIdentifier.slug ??
        problemIdentifier.title ??
        '')
      : problemIdentifier;
  ...
  const staticMatch =
    ...
    all425Problems.find(
      (p) => slugifyProblemTitle(p.title) === slugifyProblemTitle(rawIdentifier)
    ) ||
    ...
    if ((language === 'python' || language === 'javascript') && hasTestCases) {
      ...
      if (result.success) {
        ...
        testResults = testCases.map((tc) => ({
          passed: false,
          expected: tc.output,
          actual: result.error || 'Runtime Error',
          input: tc.input,
          error: result.error,
        }));
      }
          ...
          if (result.success) {
            ...
            if (parsed) {
              ...
              return res.json({
                ...
                message:
                  passed === judgedResults.length
                    ? 'All test cases passed!'
                    : `${passed}/${judgedResults.length} test cases passed`,
              });
            }
          }
...
router.post('/execute', optionalAuth, async (req, res) => {
  ...
  if (!language || typeof language !== 'string') {
    return res
      .status(400)
      .json({ success: false, output: '', error: 'Language is required', executionTime: 0 });
  }

  if (typeof code !== 'string') {
    return res
      .status(400)
      .json({ success: false, output: '', error: 'Code must be a string', executionTime: 0 });
  }

  if (!code.trim()) {
    return res
      .status(400)
      .json({ success: false, output: '', error: 'Code is required', executionTime: 0 });
  }
...
router.post('/lint', optionalAuth, (req, res) => {
  ...
  if (!language || typeof language !== 'string') {
    return res.status(400).json({
      success: false,
      errors: [{ line: 1, col: 1, severity: 'error', message: 'Language is required' }],
    });
  }
  ...
  if (typeof code !== 'string') {
    return res.status(400).json({
      success: false,
      errors: [{ line: 1, col: 1, severity: 'error', message: 'Code must be a string' }],
    });
  }

  if (code.length > MAX_LINT_CODE_LENGTH) {
      ...
      errors: [
        {
          ...
          col: 1,
          severity: 'error',
          message: `Code is too large for live linting. Limit is ${MAX_LINT_CODE_LENGTH} characters.`,
        },
      ],
    });
  }
  ...
  try {
    ...
    if (normalizedLanguage === 'javascript') {
      try {
        ...
        return res.json({
          ...
          errors: [
            { line, col, severity: 'error', message: String(err?.message || 'Syntax error') },
          ],
        });
      }
    }
    ...
    if (normalizedLanguage === 'python') {
          ...
          errors: [
            {
              ...
              col: 1,
              severity: 'warning',
              message: 'Python runtime not found for server lint checks',
            },
          ],
        });
      }
      ...
      try {
        ...
        execFileSync(pythonCmd, ['-m', 'py_compile', lintFile], {
          stdio: 'pipe',
          timeout: 8000,
          cwd: lintDir,
        });
        ...
        const stderr = lintErr?.stderr
          ? lintErr.stderr.toString()
          : lintErr?.message || 'Syntax error';
        return res.json({
          success: true,
    ...
    if (normalizedLanguage === 'c') {
          ...
          errors: [
            {
              ...
              col: 1,
              severity: 'warning',
              message: 'C compiler not found for server lint checks',
            },
          ],
        });
      }
      ...
      try {
        fs.writeFileSync(lintFile, code, 'utf-8');
        execFileSync(cCmd, ['-fsyntax-only', lintFile], {
          stdio: 'pipe',
          timeout: 8000,
          cwd: lintDir,
        });
        ...
        const stderr = lintErr?.stderr
          ? lintErr.stderr.toString()
          : lintErr?.message || 'C syntax error';
        return res.json({
          success: true,
    ...
    if (normalizedLanguage === 'cpp') {
      ...
      try {
        fs.writeFileSync(lintFile, code, 'utf-8');
        execFileSync(cppCmd, ['-fsyntax-only', lintFile], {
          stdio: 'pipe',
          timeout: 8000,
          cwd: lintDir,
        });
    ...
    if (normalizedLanguage === 'java') {
          ...
          errors: [
            {
              ...
              col: 1,
              severity: 'warning',
              message: 'Java compiler not found for server lint checks',
            },
          ],
        });
      }
      ...
      try {
        ...
        const stderr = lintErr?.stderr
          ? lintErr.stderr.toString()
          : lintErr?.message || 'Java syntax error';
        return res.json({
          success: true,
          checkedWith: 'javac',
    ...
    return res.status(500).json({
      success: false,
      errors: [
        { line: 1, col: 1, severity: 'error', message: error.message || 'Failed to lint code' },
      ],
    });
  }
});
