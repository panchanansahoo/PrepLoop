...
router.get('/analytics', authenticateToken, async (req, res) => {
  ...
  try {
    const {
      data: sessions,
      error
    } = await supabaseAdmin.from('interview_sessions').select('overall_score, company, stage, difficulty, completed_at, scores, speech_metrics, emotion_data, proctoring_violations').eq('user_id', userId).order('completed_at', {
      ascending: true
    });
    ...
    sessions.forEach(s => {
      if (!companyBreakdown[s.company]) companyBreakdown[s.company] = {
        count: 0,
        totalScore: 0
      };
    ...
    sessions.forEach(s => {
      if (!stageBreakdown[s.stage]) stageBreakdown[s.stage] = {
        count: 0,
        totalScore: 0
      };
    ...
    const recentImprovement = Math.round(last5.reduce((a, b) => a + b, 0) / last5.length - first5.reduce((a, b) => a + b, 0) / first5.length);
    res.json({
      totalSessions,
      averageScore,
      scoreTrend,
