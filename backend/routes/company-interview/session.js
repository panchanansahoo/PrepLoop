...
router.post('/start', optionalAuth, async (req, res) => {
  ...
  const resolvedTotalQuestions = Number.isFinite(requestedCount) && requestedCount >= 4 && requestedCount <= 12 ? Math.round(requestedCount) : normalizedAdvanced.questionCount;
  ...
  if (isFresherTechnical) {
    ...
    return res.json(withRuntime({
      ...
      context: {
        ...
        advancedOptions: {
          ...normalizedAdvanced,
          questionCount: FRESHER_INTERVIEW_TOTAL_QUESTIONS
        },
        interviewRuntimeMode: resolvedRuntimeMode
      },
  ...
  if (staticQuestions.length > 0) {
    ...
    return res.json(withRuntime({
      ...
      context: {
        ...
        advancedOptions: {
          ...normalizedAdvanced,
          questionCount: staticQuestions.length
        },
        interviewRuntimeMode: resolvedRuntimeMode
      },
      tips: stage === 'HR' ? ['Introduce yourself clearly and keep your answer structured.', 'Use one concrete example when possible.'] : ['Start with a direct answer, then explain your reasoning.', 'Mention trade-offs and edge cases where relevant.'],
      interviewerReaction: 'greeting',
      thinkTime: 30,
      questionSource: 'scripted',
      questionMeta: {
        id: `${String(stage).toLowerCase()}-q-1`,
    ...
    if (questionSource === 'database' && questionBank.length > 0) {
      ...
      return res.json(withRuntime({
        ...
        questionMeta: {
          ...
          frequencyScore: firstQ.frequencyScore
        },
...
Respond as JSON:
{
  ...
  "interviewerReaction": "greeting"
          ...
          role: 'user',
          content: `Start the ${stage} interview for ${role} at ${company}.`
        }],
        response_format: {
          type: 'json_object'
        },
...
router.post('/save-session', authenticateToken, async (req, res) => {
  ...
  if (!userId) return res.status(200).json({
    message: 'Guest session not saved'
  });
  const {
    type = 'single',
    company,
...
router.get('/sessions', authenticateToken, async (req, res) => {
  ...
  try {
    let query = supabaseAdmin.from('interview_sessions').select('id, session_type, company, role, stage, difficulty, overall_score, completed_at, created_at').eq('user_id', userId).order('completed_at', {
      ascending: false
...
router.get('/sessions/:id', authenticateToken, async (req, res) => {
  ...
  try {
    const {
      data,
      error
    } = await supabaseAdmin.from('interview_sessions').select('*').eq('id', req.params.id).eq('user_id', userId).single();
    if (error) throw error;
