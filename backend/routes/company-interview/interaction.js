...
router.post('/follow-up', optionalAuth, async (req, res) => {
  ...
  const safeQuestionNumber = Number.isFinite(parsedQuestionNumber) && parsedQuestionNumber >= 1 ? Math.floor(parsedQuestionNumber) : 2;
  const parsedTotalQuestions = Number(totalQuestions);
  const safeTotalQuestions = Number.isFinite(parsedTotalQuestions) && parsedTotalQuestions >= 1 ? Math.floor(parsedTotalQuestions) : 8;
  ...
  const normalizedAdvanced = resolveResumeInterviewModeForExperience(normalizeAdvancedOptions(advancedOptions), experienceLevel, resumeContext);
  ...
  if (isFresherHR) {
    ...
    const feedbackOptions = ['That\'s great to hear! Thank you for sharing that.', 'I appreciate your openness and clear example there.', 'That shows real self-awareness and growth mindset.', 'Wonderful! You gave a thoughtful and genuine answer.', 'Excellent! You\'ve demonstrated great interpersonal awareness.', 'That\'s really thoughtful. I like how you framed that.'];
  ...
  if (isFresherTechnical) {
    ...
    const feedbackOptions = ['That\'s a solid approach! I like how you explained that.', 'Good thinking! I can see you have hands-on experience.', 'I appreciate the detail there. Let\'s explore a bit more.', 'Nice! You covered the key points clearly.', 'Excellent! You have a practical understanding.'];
    feedbackMessage = deterministicPick(feedbackOptions, qNum);
    let followUpQuestion = '';
    let closingRemark = undefined;
    let isComplete = false;
  ...
  if (staticQuestions.length > 0) {
    ...
    const resolvedNextQuestion = isLastQuestion ? '' : nextQuestion || staticQuestions[safeQuestionIdx] || '';
    return res.json(withRuntime({
      feedback: stage === 'HR' ? 'That was a thoughtful response. Let us continue the discussion with the next HR question.' : 'Good response. Let us continue with the next technical question.',
      ...
      strengths: stage === 'HR' ? ['Clear communication', 'Relevant examples'] : ['Structured thinking', 'Technical clarity'],
      improvements: stage === 'HR' ? ['Add one concrete example', 'Keep answers concise and outcome-focused'] : ['Mention complexity clearly', 'Call out edge cases earlier'],
      interviewerReaction: isLastQuestion ? 'encouraging' : 'probing',
      thinkTime: 20,
      hint: stage === 'HR' ? 'Answer naturally and relate your response to your experience.' : 'Explain your reasoning step by step, then mention trade-offs.',
      difficultyLevel: 'medium',
      ...
      questionMeta: {
        id: `${String(stage).toLowerCase()}-q-${Math.min(safeQuestionNumber, staticQuestions.length)}`,
        track: String(stage).toLowerCase(),
        sequence: Math.min(safeQuestionNumber, staticQuestions.length)
      }
    }));
  }
  ...
  const getFresherTopic = qNum => {
    switch (qNum) {
      case 2:
        return "Resume & Experience: Ask them to elaborate on a project listed in their resume (or a general past project if no resume).";
      case 3:
        return "Technical Skills: Ask them about a specific language or framework they are confident in, and how they used it practically.";
      case 4:
        return "Object-Oriented Programming (OOP) Core: Ask them to explain a foundational OOP concept (like the 4 pillars) with a real-world example.";
      case 5:
        return "OOP Applied: Ask them to compare two OOP concepts (like Interface vs Abstract Class) and when to use each.";
      case 6:
        return "SQL / Databases: Ask them about foundational SQL concepts (like JOINs or Normalization).";
      case 7:
        return "Database Design: Ask them about Primary/Foreign Keys, Indexes, or typical database performance optimization.";
      case 8:
        return "Programming Language / Core: Ask them to explain a core programming language concept (e.g., pass-by-value vs pass-by-reference).";
      case 9:
        return "OS Fundamentals: Ask an operating system fundamental (like Process vs Thread or Memory Management).";
      case 10:
        return "Networking / Web: Ask about REST APIs, HTTP methods, or the client-server request cycle.";
      case 11:
        return "Data Structures: Ask about a fundamental data structure (arrays, linked lists, stacks, queues, trees) and when to use it.";
      case 12:
        return "Wrap Up: Ask if they have any questions for you about the company, team, or the role.";
      case 13:
        return "Conclusion: Warmly answer the candidate's questions and strictly conclude the interview.";
      default:
  ...
  if (useRealQuestions && questionBankIds && questionBankIds.length > 0) {
    ...
    if (nextIdx < questionBankIds.length) {
      const {
        getQuestionById
      } = await import('../services/companyQuestionService.js');
      nextRealQuestion = await getQuestionById(questionBankIds[nextIdx]);
      if (nextRealQuestion) {
        realQuestionMeta = {
          id: nextRealQuestion.id,
          tags: nextRealQuestion.tags,
          difficulty: nextRealQuestion.difficulty,
          frequencyScore: nextRealQuestion.frequencyScore
        };
      }
    }
  }
  ...
  try {
    if (!groq) {
      const reactions = ["That's a solid approach! I like how you structured that.", "Interesting perspective. Let me dig a bit deeper.", "Good thinking! I can see you've worked through problems like this before.", "I appreciate the detail there. Let's explore a different angle.", "Nice! You covered the key points well.", "Great answer! I can tell you have hands-on experience with this.", "I like your thought process. Let me push you a bit further."];
      // Diverse follow-ups per stage — rotates based on questionNumber
      const stageQuestions = {
        'Technical': ['Can you explain the difference between an abstract class and an interface? When would you pick one over the other?', 'How does garbage collection work in your preferred language? What are the different algorithms?', 'What is the difference between concurrency and parallelism? Can you give a real-world example?', 'Explain how a hash map works internally. What happens during a collision?', 'What are SOLID principles? Can you walk me through each one with a quick example?', 'How would you debug a production issue where the application is running slow but CPU usage is normal?', 'What is the CAP theorem? How does it apply to database selection?', 'Explain the event loop in Node.js. How does it handle asynchronous operations?'],
        'DSA / Coding': ['Given a linked list, how would you detect if it has a cycle? Walk me through your approach.', 'How would you find the kth largest element in an unsorted array? What data structure helps here?', 'Explain how you would serialize and deserialize a binary tree. What traversal order would you use?', 'Given a string of parentheses, how would you check if they are balanced? What about multiple types?', 'How would you merge two sorted arrays in-place? Think about doing it without extra space.', 'Describe how you would implement an LRU cache. What data structures would you combine?', 'How would you find the longest increasing subsequence in an array? Discuss both approaches.', 'Given a matrix of 0s and 1s, how would you count the number of islands?'],
        'System Design': ['How would you design a real-time chat application like WhatsApp? Think about message delivery guarantees.', 'Walk me through designing a news feed system like Instagram. How would you handle ranking?', 'How would you design a rate limiter for an API? What algorithms could you use?', 'Design a notification system that handles millions of users. How do you handle delivery at scale?', 'How would you design a key-value store like Redis? What about persistence and replication?', 'Walk me through designing a search autocomplete system. How would you optimize for latency?', 'How would you design a video streaming platform? Think about CDN and adaptive bitrate.', 'Design a parking lot system. Start with the object model, then think about scaling.'],
        'Behavioral': ['Tell me about a time you worked on a challenging group project in college. How did you handle disagreements?', 'Describe a hackathon, competition, or event you participated in. What was your role?', 'Can you share an example where you had to learn something completely new under a tight deadline?', 'Tell me about a project that didn\'t go as planned. What did you learn from it?', 'Describe a time when you had to balance multiple assignments or deadlines. How did you prioritize?', 'Share an experience where you helped a classmate or junior understand something difficult.', 'Tell me about a time you took initiative on something — maybe a club, project, or side hustle.', 'Describe how you handled a situation where you were unsure how to approach a problem or assignment.'],
        'HR': ['What motivates you to pursue a career in tech? What gets you excited about building things?', 'Where do you see yourself in 3 to 5 years? How does this role fit into that vision?', 'What made you choose your branch or field of study? Do you enjoy it?', 'What kind of work environment do you think you\'d thrive in? Do you prefer working solo or in teams?', 'Tell me about a value or principle that guides how you work and learn.', 'What are you most proud of from your time in college — could be academic or extracurricular!', 'How would your friends or classmates describe you in three words?', 'What questions do you have for me about the team, the role, or what it\'s like to work at this company?'],
        'Managerial': ['Have you ever had to coordinate or lead a team? Maybe a college project, club, or event?', 'Tell me about a time you had to convince your team to go with your idea. How did you handle it?', 'How do you keep yourself and your team motivated during a stressful period, like exams or a project deadline?', 'If you had to organize a technical event or workshop, how would you plan it?', 'How would you handle a situation where a team member isn\'t contributing fairly in a group project?', 'What is your approach to managing your time when you have multiple deadlines?', 'Do you enjoy mentoring juniors? Tell me about a time you helped someone learn something.', 'Tell me about a tough decision you made — maybe about choosing between two projects, events, or even career options.'],
        'OA': ['Given an array of integers, find the contiguous subarray with the maximum sum. Explain your approach.', 'How would you determine if a string is a valid palindrome, considering only alphanumeric characters?', 'Write an algorithm to find all permutations of a given string. What is the time complexity?', 'Given two sorted arrays, find the median of the combined array in O(log n) time.', 'How would you implement a stack that supports getMin() in O(1) time?', 'Given a 2D grid, find the shortest path from top-left to bottom-right. What algorithm would you use?', 'Design an algorithm to rotate a matrix 90 degrees clockwise in-place.', 'How would you detect a duplicate in an array of n+1 integers where each integer is between 1 and n?']
      };
      ...
      return res.json(withRuntime({
        ...
        strengths: [deterministicPick(['Clear communication', 'Structured thinking', 'Good examples'], safeQuestionNumber), deterministicPick(['Technical depth', 'Problem-solving mindset', 'Practical approach'], safeQuestionNumber + 1)],
        improvements: [deterministicPick(['Add more specific metrics', 'Consider edge cases', 'Discuss trade-offs'], safeQuestionNumber), deterministicPick(['Mention real-world experience', 'Think about scalability', 'Explore alternatives'], safeQuestionNumber + 1)],
    ...
    if (fresherScriptMode) {
      ...
      if (isLastQuestion) {
        ...
        if (!shouldClose) {
          ...
          return res.json(withRuntime({
            ...
            questionMeta: {
              id: 'fresher-q-12-loop',
              track: 'fresher-hr-tech',
              sequence: 12
            }
          }));
        }
        return res.json(withRuntime({
          ...
          questionSource: 'ai-scripted',
          questionMeta: {
            id: `fresher-q-${effectiveTotalQuestions + 1}`,
            track: 'fresher-hr-tech',
            sequence: effectiveTotalQuestions + 1
          }
        }));
      }
      ...
      return res.json(withRuntime({
        ...
        questionSource: 'ai-scripted',
        questionMeta: {
          id: `fresher-q-${safeQuestionNumber}`,
          track: 'fresher-hr-tech',
          sequence: safeQuestionNumber
        }
      }));
    }
...
Respond as JSON:
{
  ...
  "adaptiveNote": "Brief explanation of difficulty adjustment"${code ? ',\n  "codeFeedback": {"correctness": "pass|fail|partial", "timeComplexity": "O(...)", "spaceComplexity": "O(...)", "quality": 0-10, "issues": ["issue1"], "optimizedApproach": "brief suggestion"}' : ''}
    ...
    res.json(withRuntime({
      ...
      questionMeta: fresherScriptMode ? {
        id: `fresher-q-${safeQuestionNumber}`,
        track: 'fresher-hr-tech',
        sequence: safeQuestionNumber
      } : realQuestionMeta || null,
    ...
    const technicalFallbackQuestions = experienceLevel === 'fresher' ? ['Can you connect that to a class project, internship, or side project you worked on?', 'Can you give one concrete example from something you built or studied?', 'Can you walk me through a specific project or coursework example that shows that in practice?'] : ['Can you walk me through your approach step by step, including trade-offs?', 'What alternative would you consider, and why?', 'How would you apply that in a real system or team setting?'];
    ...
    const defaultFallbackByExperience = experienceLevel === 'fresher' ? pickTechnicalFallbackQuestion(questionNumber) : 'Good answer. Can you compare one alternative approach and explain the trade-off?';
...
Respond as JSON:
{
  ...
  "keyTopics": ["Topic 1", "Topic 2", "Topic 3"]
          ...
          role: 'user',
          content: `I'm stuck on: "${currentQuestion}". Can you give me a hint?`
        }],
        response_format: {
          type: 'json_object'
        },
  ...
  try {
    if (!groq || !question) {
      return res.status(400).json({
        error: 'Missing requirements for rephrase'
      });
    }
    const completion = await aiCallWithRetry({
      operation: () => groq.chat.completions.create({
...
Output ONLY a JSON object with this shape:
{
  "rephrased": "The simpler version of the question"
    ...
    res.status(500).json({
      error: 'Failed to rephrase question'
    });
  }
});
