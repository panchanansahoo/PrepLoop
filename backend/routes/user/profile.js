...
router.get("/profile", authenticateToken, async (req, res) => {
  try {
    ...
    const {
      data: profile,
      error
    } = await supabaseAdmin.from("profiles").select("*").eq("id", req.user.id).maybeSingle();
...
router.get('/portfolio/public/:slug', async (req, res) => {
  try {
    ...
    let {
      data: profile,
      error
    } = await supabaseAdmin.from('profiles').select('*').eq('custom_url', slug).maybeSingle();
    if (error) {
...
router.put("/profile", authenticateToken, async (req, res) => {
  try {
    ...
    const {
      data,
      error
    } = await supabaseAdmin.from("profiles").update(updates).eq("id", req.user.id).select().maybeSingle();
    if (error) {
...
router.post('/profile/avatar', authenticateToken, upload.single('avatar'), async (req, res) => {
  try {
    ...
    const {
      error: uploadError
    } = await supabaseAdmin.storage.from('avatars').upload(filePath, file.buffer, {
...
router.post('/profile/claim-url', authenticateToken, async (req, res) => {
  try {
    ...
    if (!/^[a-z0-9\-]{3,30}$/.test(requested)) {
      return res.status(400).json({
        error: 'Invalid custom_url format. Use 3-30 chars: a-z, 0-9, -'
      });
    }
    try {
      const result = await claimCustomUrl(supabaseAdmin, req.user.id, requested);
      if (!result.success) {
        if (result.error === 'taken') return res.status(409).json({
          error: 'custom_url already taken'
        });
        return res.status(400).json({
          error: 'Invalid custom_url'
        });
      }
      ...
      return res.status(500).json({
        error: 'Failed to claim custom_url'
      });
    }
  } catch (error) {
...
router.get("/settings", authenticateToken, async (req, res) => {
  try {
    const {
      data: profile,
      error
    } = await supabaseAdmin.from("profiles").select("*").eq("id", req.user.id).single();
    if (isProfilesAccessBlocked(error)) {
      return res.json({
        settings: {
          full_name: req.user.user_metadata?.full_name || '',
...
router.put("/settings", authenticateToken, async (req, res) => {
  try {
    ...
    const {
      data,
      error
    } = await supabaseAdmin.from("profiles").update(updates).eq("id", req.user.id).select().single();
...
router.post("/preferences", authenticateToken, async (req, res) => {
  try {
    ...
    if (Object.keys(updates).length > 0) {
      const {
        error
      } = await supabaseAdmin.from("profiles").update(updates).eq("id", req.user.id);
      if (error) throw error;
    }
    res.json({
      success: true,
    ...
    if (isProfilesAccessBlocked(error)) {
      return res.status(503).json({
        error: "Preferences update is temporarily unavailable",
        degraded: true
      });
    }
    res.status(500).json({
